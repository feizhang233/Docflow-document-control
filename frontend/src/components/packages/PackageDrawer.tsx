...
export function PackageDrawer({item,configs,workflowConfig,saving,readOnly=false,onUpdate,onClose}:{item:Package|null;configs:ColumnConfig[];workflowConfig:WorkflowConfig;saving:boolean;readOnly?:boolean;onUpdate:(data:Partial<PackageInput>)=>void;onClose:()=>void}){
  ...
  return createPortal(<div className="drawer-layer" role="dialog" aria-modal="true" aria-label="Document details"><div className="drawer-backdrop" onClick={onClose}/><aside className="detail-drawer"><header><div><span className="eyebrow">Document details</span><h2>{item.document_number}</h2></div><button className="icon-button" onClick={onClose}><X size={19}/></button></header><div className="drawer-body">
    ...
    <section className="drawer-section notes-section"><div className="section-heading"><h4>Notes</h4>{!readOnly&&<button className="secondary-button" disabled={saving||notes===item.notes} onClick={()=>onUpdate({notes})}><Save/>Save notes</button>}</div><textarea className="resize-none" value={notes} onChange={e=>setNotes(e.target.value)} readOnly={readOnly} placeholder="Add document control notes, decisions or follow-up context…"/></section>
    <section className={`drawer-section submission-slider-section ${item.is_abandoned?'disabled':''}`}>
      ...
      <SubmissionSlider steps={submissionSteps} value={sliderValue} onChange={setSliderValue} onCommit={commitSlider} disabled={item.is_abandoned||saving||readOnly}/>
    </section>
